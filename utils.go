...
func (b *BrowserSteps) GetCurrentWindowInnerSize() (*selenium.Size, error) {
	widthI, err := b.GetWebDriver().ExecuteScript("return window.innerWidth", nil)
	if err != nil {
		return nil, err
	}
	...
	heightI, err := b.GetWebDriver().ExecuteScript("return window.innerHeight", nil)
	if err != nil {
		return nil, err
	}
...
func (b *BrowserSteps) GetCurrentWindowScroll() (*selenium.Point, error) {
	scrollXI, err := b.GetWebDriver().ExecuteScript("return window.scrollX", nil)
	if err != nil {
		return nil, err
	}
	...
	scrollYI, err := b.GetWebDriver().ExecuteScript("return window.scrollY", nil)
	if err != nil {
		return nil, err
	}
